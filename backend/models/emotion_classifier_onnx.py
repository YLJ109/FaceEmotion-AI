...
class EmotionClassifierONNX:
    ...
    # 灰度图归一化参数
    GRAYSCALE_MEAN = np.array([0.5], dtype=np.float32)
    GRAYSCALE_STD = np.array([0.5], dtype=np.float32)

    def __init__(self, model_path: str, use_quantized: bool = False):
        self.use_quantized = use_quantized
        self.img_size = 96  # 模型期望96x96输入
    ...
    def _preprocess(self, image: np.ndarray) -> np.ndarray:
        ...
        tensor = (tensor - self.GRAYSCALE_MEAN) / self.GRAYSCALE_STD
        
        # 添加通道维度和batch维度: [H, W] -> [1, 1, H, W]
        tensor = np.expand_dims(tensor, axis=0)  # 添加通道维度
